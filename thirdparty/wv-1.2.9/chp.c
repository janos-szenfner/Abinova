...
wvMergeCHPXFromBucket (CHPX * dest, UPXF * src)
{
    ...
    /*
       ...
       AbiWord: both walks are bounded -- a sprm whose operand would run past
       the end of its grpprl stops the walk, so len can never wrap (it was a
       U8 that overflowed above 255 bytes) and no truncated operand is ever
       queued into the tree.
     */
    ...
    while (testn != NULL && (U32) (dpointer - grpprl) < len)
      {
	  ...
	  if ((U32) i + 2 > len - (U32) (dpointer - grpprl))
	    {
    ...
    wvFree (dest->grpprl);
    dest->grpprl = grpprl;
    dest->cbGrpprl = (U16) (dpointer - grpprl);
...
wvUpdateCHPXBucket (UPXF * src)
{
    ...
    while (i < src->cbUPX)
	  ...
	  if (temp > avail)
	    {
		temp = avail;	/* truncated operand: take what remains */
		i = src->cbUPX;
	    }
	  pointer += temp;
	  wvTrace (("Mp dis is %d\n", pointer - src->upx.chpx.grpprl));
...
wvGetCHPX (wvVersion ver, CHPX * item, U8 * page, U16 * pos)
{
    ...
    /* AbiWord: page is a 512-byte FKP sector -- clamp the count so the
       memcpy below can never read past its end */
    if (*pos > 512)
	item->cbGrpprl = 0;
    else if (item->cbGrpprl > 512 - *pos)
	item->cbGrpprl = (U8) (512 - *pos);
    if (item->cbGrpprl > 0)
      {
