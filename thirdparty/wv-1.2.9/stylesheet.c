...
wvGetSTD (STD * item, U16 baselen, U16 fixedlen, wvStream * fd)
{
	...
	*(item->xstzName) = 0;
    ...
    for (i = 0; i < item->cupx; i++)
	  ...
	  /* AbiWord: never read past the end of this std record */
	  if (pos >= (int) fixedlen)
	    {
	  ...
	  /* clamp cbUPX to the bytes remaining in the record */
	  if (pos + item->grupxf[i].cbUPX > (int) fixedlen)
	      item->grupxf[i].cbUPX = (U16) (fixedlen - pos);
	  ...
	  if ((item->cupx == 1) || ((item->cupx == 2) && (i == 1)))
		...
		pos += 2;
		/* AbiWord: cbUPX < 2 would underflow the size below */
...
wvGetSTSH (STSH * item, U32 offset, U32 len, wvStream * fd)
{
     ...
     */
    /* AbiWord: std[10] is out of bounds when cstd <= 10 */
    if (item->Stshi.cstd > 10 && item->std[10].istdBase == istdNil)
      {
    ...
    while (j < 11)
	  ...
	  for (i = 0; i < item->Stshi.cstd; i++)
	    {
		/* AbiWord: istdBase comes from the file -- bounds-check it
		   before indexing chains1 */
		if ((chains1[i] != istdNil) &&
		    (chains1[i] < item->Stshi.cstd) &&
		    (chains1[chains1[i]] == istdNil))
		  {
		      chains2[i] = istdNil;
		      wvTrace (("Generating istd no %d\n", i));
		      wvGenerateStyle (item, i, word6);
