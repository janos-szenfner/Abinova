...
wvGetPAPX_FKP (wvVersion ver, PAPX_FKP * fkp, U32 pn, wvStream * fd)
{
    ...
    if (wvStream_read (page, WV_PAGESIZE, 1, fd) < 1)
      {
    ...
    /* AbiWord: crun is file-controlled -- (crun+1)*4 bytes of rgfc must
       fit inside the 512-byte page, so clamp it */
    if (fkp->crun > (WV_PAGESIZE - 1) / 4 - 1)
	fkp->crun = (WV_PAGESIZE - 1) / 4 - 1;
