		       ...
		       int nobte, wvStream * fd)
{
    ...
    if (fkp->rgfc == NULL)	/* AbiWord: unreadable FKP page */
	return (1);
    ...
    while (fkp->rgfc[fkp->crun] <= currentfc)
	  ...
	  entry.pn++;
	  wvGetPAPX_FKP (ver, fkp, entry.pn, fd);
	  if (fkp->rgfc == NULL)	/* ran off the end of the table stream */
	      return (1);
      }
...
{
    U32 i = 0;
    /* AbiWord: need at least a first and a limit FC; nopos < 2 would
       index rgfc out of bounds */
    if (rgfc == NULL || nopos < 2)
	return (1);
    while (i < nopos - 1)
      {
	  wvTrace (
