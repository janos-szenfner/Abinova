...
private:
  ...
  std::stack<UT_uint32>	m_stackFmtStartIndex;
  ...
  virtual void startElement (const gchar * name, const gchar ** atts) override
  {
	...
	if (abi_sty) {
	  ...
	  if (abi_sty->getColBreakBefore () || abi_sty->getPageBreakBefore ()) {
	    ...
	    UT_UCS4Char ucs = (abi_sty->getColBreakBefore () ? UCS_VTAB : UCS_FF);
...
private:
  ...
  {
    if (m_stackFmtStartIndex.empty())
      return;

    UT_uint32 start = m_stackFmtStartIndex.top();
    m_stackFmtStartIndex.pop();
