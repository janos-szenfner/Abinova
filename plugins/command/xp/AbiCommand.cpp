...
AbiCommand::parseTokens (UT_GenericVector<const UT_UTF8String*> * pToks)
{
	...
	else if (strcmp (pCom0->utf8_str (), "findnext") == 0)
	{
		if (m_pCurView && (pToks->getItemCount () > 1))
		{
			...
			const UT_UCS4Char *pUCSFind =
				reinterpret_cast < UT_UCS4Char * >(UT_calloc (pFind->size () + 1, sizeof (UT_UCS4Char)));
...
AbiCommand::replaceAll (const UT_GenericVector<const UT_UTF8String*> * pToks)
{
	if (m_pCurView != nullptr)
	{
		...
		UT_UCS4Char *pUCSFind =
			reinterpret_cast < UT_UCS4Char * >(UT_calloc (pFind->size () + 1, sizeof (UT_UCS4Char)));
		UT_UCS4Char *pUCSReplace =	reinterpret_cast < UT_UCS4Char *	>(UT_calloc (pReplace->size () + 1, sizeof (UT_UCS4Char)));
...
AbiCommand::replaceNext (const UT_GenericVector<const UT_UTF8String*> * pToks)
{
	if (m_pCurView != nullptr)
	{
		...
		UT_UCS4Char *pUCSFind =
			reinterpret_cast < UT_UCS4Char *	>(UT_calloc (pFind->size () + 1, sizeof (UT_UCS4Char)));
		UT_UCS4Char *pUCSReplace =
			reinterpret_cast < UT_UCS4Char *	>(UT_calloc (pReplace->size () + 1, sizeof (UT_UCS4Char)));
		bool bEOD = false;
