...
AP_UnixDialog_Tab::_constructWindow ()
{
	...
	m_pBuilder = newDialogBuilderFromResource("ap_UnixDialog_Tab.ui");
...
{
	gchar *text =  gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT (m_cobAlignment));
...
{
	gchar *text =  gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT (m_cobLeader));
...
AP_UnixDialog_Tab::_setDefaultTabStop (const gchar *defaultTabStop)
{
	...
	UT_return_if_fail (defaultTabStop && *defaultTabStop);
