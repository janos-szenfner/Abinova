...
//
		...
		{
		      const pf_Frag_Strux* sdhLow = pAuto->getFirstItem();
		      const pf_Frag_Strux* sdhHigh = pAuto->getLastItemInHierarchy();
 ...
 * Preview.
 */
UT_UCS4Char * AP_Dialog_Lists::getListLabel(UT_sint32 itemNo)
{
	UT_ASSERT_HARMLESS(itemNo < 4);
	static UT_UCS4Char lab[80];
	const UT_UCS4Char * tmp  = m_pFakeAuto->getLabel(m_pFakeSdh[itemNo]);
	if(tmp == nullptr)
	{
		return nullptr;
	}
...
void AP_Lists_preview::drawImmediate(const UT_Rect* clip)
{
  ...
  UT_sint32 indent = m_gc->tlu(static_cast<UT_sint32>( z*(m_fAlign+m_fIndent)));
	...
	for(i=0; i<4; i++)
	{
		UT_UCS4Char * lv = getLists()->getListLabel(i);
		UT_sint32 len =0;
	...
	for(i=0; i<8; i++)
	{
		...
		if((i & 1) == 0)
		{
			...
			// Draw the text
			//
			UT_UCS4Char * lv = getLists()->getListLabel(i/2);
			UT_sint32 len =0;
