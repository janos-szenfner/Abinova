...
void s_RTF_ListenerWriteDoc::_outputData(const UT_UCS4Char * data, UT_uint32 length, PT_DocPosition pos, bool bIgnorePosition)
{
	UT_String sBuf;
...
// custom abinodiroverride keyword
		...
		default:
			...
			} else if (!m_pie->m_atticFormat)
			{
				...
				*/
				UT_UCS4Char c = *pData++;
				UT_UCS4Char lc = XAP_EncodingManager::get_instance()->try_UToWindows(c);
				if (lc==0 || lc >255)
				{
					/*
...
{
	const UT_UCS4Char * szFieldValue = _getFieldValue();
 ...
 */
const UT_UCS4Char * s_RTF_ListenerWriteDoc::_getFieldValue(void)
{
//
...
{
	const pf_Frag_Strux* sdhCell = m_pDocument->getCellStruxFromRowCol(m_Table.getTableStrux(), true, PD_MAX_REVISION, row, col);
	if(sdhCell == nullptr)
	{
		return -1;
	}
	const char * szRight;
	m_pDocument->getPropertyFromStrux(sdhCell,true,PD_MAX_REVISION,"right-attach",&szRight);
...
//
	...
	for(i=0; i < m_Table.getNumCols(); i = iNext)
	{
		...
		{
			const pf_Frag_Strux* cellStrux = m_pDocument->getCellStruxFromRowCol(m_Table.getTableStrux(), true, PD_MAX_REVISION,
																		  row,i);
			UT_ASSERT_HARMLESS(cellStrux);
			if (cellStrux)
			{
				m_pDocument->miniDump(cellStrux, 8);
			}
			UT_ASSERT_HARMLESS(UT_SHOULD_NOT_HAPPEN);
			iNext = i+1;
		}
...
void s_RTF_ListenerWriteDoc::_open_table(PT_AttrPropIndex api,bool bIsCell)
{
	...
	if(bIsCell)
	{
		...
		bool b = m_pDocument->getMutStruxOfTypeFromPosition(posCell, PTX_SectionTable, &sdhTable);
		UT_return_if_fail(b);
		api = m_pDocument->getAPIFromStrux(sdhTable);
