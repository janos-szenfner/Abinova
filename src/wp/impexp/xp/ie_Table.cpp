...
{
	m_pDoc->getRowsColsFromTableStrux(m_TableSDH, true, PD_MAX_REVISION, &m_iNumRows, &m_iNumCols);
}
 ...
 */
pf_Frag_Strux* ie_Table::getTableStrux(void) const
{
	UT_return_val_if_fail(!m_sLastTable.empty(), nullptr);
	ie_PartTable * pPT = m_sLastTable.top();
	if(pPT)
	{
		return pPT->getTableStrux();
	}
	return nullptr;
}
...
void ie_Table::setCellRowCol(UT_sint32 row, UT_sint32 col)
{
	...
	if(sdhStart == nullptr)
	{
		sdhStart = pPT->getTableStrux();
	}
	pf_Frag_Strux* cellSDH = m_pDoc->getCellMutStruxFromRowCol(sdhStart,true,PD_MAX_REVISION,row,col);
	if(cellSDH == nullptr)
	{
		sdhStart = pPT->getTableStrux();
		cellSDH = m_pDoc->getCellMutStruxFromRowCol(sdhStart,true,PD_MAX_REVISION,row,col);
	}
	m_sdhLastCell = cellSDH;
	if(cellSDH != nullptr)
	{
		PT_AttrPropIndex api = m_pDoc->getAPIFromStrux(cellSDH);
		pPT->setCellApi(api);
	}
}
 ...
 */
pf_Frag_Strux* ie_imp_cell::getCellStrux(void) const
{
	return m_cellSDH;
}
 ...
 * Set Cell SDH 
 */
void ie_imp_cell::setCellStrux(pf_Frag_Strux* cellSDH)
{
	m_cellSDH = cellSDH;
}
 ...
 */
pf_Frag_Strux* ie_imp_table::getTableStrux(void) const
{
	return m_tableSDH;
}
 ...
 */
void ie_imp_table::setTableStrux(pf_Frag_Strux* sdh)
{
	m_tableSDH = sdh;
...
void ie_imp_table::writeAllCellPropsInDoc(void)
{
	...
	for(i=0; i< m_vecCells.getItemCount();i++)
	{
		...
		if(pCell->isMergedAbove() && (pCell->getCellStrux() != nullptr))
		{
			...
			pf_Frag_Strux* cellSDH = pCell->getCellStrux();
			UT_return_if_fail(cellSDH != nullptr);
			pf_Frag_Strux* nextSDH = nullptr;
			m_pDoc->getNextMutStrux(cellSDH,&nextSDH);
		...
		if(pCell->isMergedLeft() && (pCell->getCellStrux() != nullptr))
		{
...
void ie_imp_table::deleteRow(UT_sint32 row)
{
	...
	for(i= m_vecCells.getItemCount() -1; i>=0; i--)
	{
		...
		if(pCell->getRow() == row)
		{
			...
			if(pCell->getCellStrux() != nullptr)
			{
				pf_Frag_Strux* cellSDH = pCell->getCellStrux();
				pf_Frag_Strux* endCellSDH = m_pDoc->getEndCellMutStruxFromCellStrux(cellSDH);
				...
				else
				{
					...
					while(!bDone)
					{
						...
						m_pDoc->getNextMutStrux(sdh,&nextsdh);
...
bool IE_Imp_TableHelper::tableStart (void)
{
	if(m_pfsInsertionPoint == nullptr)
	{
		if(m_style.size() == 0)
		{
			if (!getDoc()->appendStrux (PTX_SectionTable, PP_NOPROPS))
				return false;
		}
		else
		{
	...
	else
	{
		...
		if(m_style.size() == 0)
		{
			getDoc()->insertStruxBeforeFrag(pf,PTX_SectionTable, PP_NOPROPS);
		}
		else
		{
			const PP_PropertyVector atts = {
		...
		getDoc()->getPrevMutStruxOfType(static_cast<pf_Frag_Strux *>(pf), PTX_EndTable, &sdhEnd);
...
bool IE_Imp_TableHelper::tdEnd(void) const
{
	...
	pf_Frag * pf = static_cast<pf_Frag *>(m_pfsInsertionPoint);
	getDoc()->insertStruxBeforeFrag(pf, PTX_Block, PP_NOPROPS);
	return true;
}
 ...
 bool IE_Imp_TableHelper::tdStart (UT_sint32 rowspan, UT_sint32 colspan, const char * style, pf_Frag_Strux * pfsThis)
{
	...
	pf_Frag_Strux* sdhCell = nullptr;
	getDoc()->getPrevMutStruxOfType(static_cast<pf_Frag_Strux *>(pf), PTX_SectionCell, &sdhCell);
	...
	if(pfsThis == nullptr)
		{
			...
			getDoc()->getPrevMutStruxOfType(static_cast<pf_Frag_Strux *>(pf), PTX_EndCell, &sdhIns);
...
bool IE_Imp_TableHelper::BlockFormat (const PP_PropertyVector & attributes)
{
	...
	getDoc()->getPrevMutStruxOfType(pfs, PTX_Block, &sdh);
	getDoc()->changeStruxFormatNoUpdate(PTC_AddFmt, sdh, attributes);
	return true;
}

bool IE_Imp_TableHelper::Inline (const UT_UCS4Char * ucs4_str, UT_sint32 length)
{
	if(!m_bBlockInsertedForCell)
		{
			Block(PTX_Block, PP_NOPROPS);
...
bool IE_Imp_TableHelper::Object (PTObjectType pto, const PP_PropertyVector & attributes)
{
	if(!m_bBlockInsertedForCell)
		{
			Block(PTX_Block, PP_NOPROPS);
...
bool IE_Imp_TableHelperStack::Block (PTStruxType pts, const PP_PropertyVector & attributes)
{
	IE_Imp_TableHelper * th = top ();
	if (th)
...
bool IE_Imp_TableHelperStack::Inline (const UT_UCS4Char * ucs4_str, UT_sint32 length)
{
	IE_Imp_TableHelper * th = top ();
	if (th)
...
bool IE_Imp_TableHelperStack::Object (PTObjectType pto, const PP_PropertyVector & attributes)
{
	IE_Imp_TableHelper * th = top ();
	if (th)
