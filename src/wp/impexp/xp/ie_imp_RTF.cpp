...
{
	while(!m_pasteTableStack.empty())
	{
		ABI_Paste_Table * pPaste = m_pasteTableStack.top();
		m_pasteTableStack.pop();
		if(pPaste != nullptr)
		{
...
//
				...
				{
					getDoc()->getPropertyFromStrux(sdhCell,
												 true,
												 PD_MAX_REVISION,
												 "top-attach", &szVal);
					UT_ASSERT(szVal);
...
//
	...
	m_iStackLevelAtRow = m_stateStack.size();
	m_bDoCloseTable = false;
	m_iNoCellsSinceLastRow = 0;
}
...
void IE_Imp_RTF::HandleNote(void)
{
	...
	m_iDepthAtFootnote = m_stateStack.size();
...
UT_Error IE_Imp_RTF::_parseText()
{
	...
	/* m_stateStack.size() == 0 if the functions PushRTFState and PopRTFState
	...
	UT_ASSERT(m_stateStack.empty() || (iRTFStackDepth > 0));
...
//	if( ok && m_bFrameOpen && (static_cast<UT_sint32>(m_stateStack.size()) < m_iStackDepthAtFrame))	
//	{
//		HandleEndShape();
//	}
	return ok;
...
//
bool IE_Imp_RTF::ParseChar(UT_UCS4Char ch,bool no_convert)
{
...
// Close the previous table. This should always work.
	...
	case RTF_KW_u:
	{
		...
		if ((unsigned) param >= 0xDC00 && (unsigned) param <= 0xDFFF)
		{
			...
			bResult = ParseChar(static_cast<UT_UCS4Char>(param));
		m_currentRTFState.m_unicodeInAlternate = m_currentRTFState.m_unicodeAlternateSkipCount;
		return bResult;
	}
	case RTF_KW_up:
							...
							MAX_KEYWORD_LEN))
			{
				...
				case RTF_KW_abitableprops:
					...
					if(m_iIsInHeaderFooter == 0)
					{
						...
						if(pView->isHdrFtrEdit() && (pView->isInTable() || m_pasteTableStack.size() == 2) )
						{
//
...
//
				...
				case RTF_KW_shpinst:
					...
					SkipCurrentGroup();
					/*
					m_iStackDepthAtFrame = m_stateStack.size();
					m_bFrameOpen = true;
					*/
					return true;
					break;
...
//
				...
				case RTF_KW_annotation:
				{
					...
					//	return false;
					//	}
					m_pAnnotation->m_iRTFLevel = m_stateStack.size();
	...
	else
	{
		...
		if(bAbiList || bWord97List )
		{
			...
			*/
			...
			bSuccess = getDoc()->getMutStruxOfTypeFromPosition(m_dposPaste, PTX_Block, &sdh_cur);
			///
...
struct SFontTableState {
	enum DataType: uint8_t { MainFontName=0, AltFontName=1, Panose=2};
	DataType iCurrentInputData;// Are we reading the main name, the alt name or panose?
...
//		m_ctMoveID = moveid;
		pf_Frag_Strux* sdh;
		bool bResult = getDoc()->getMutStruxOfTypeFromPosition( m_dposPaste, PTX_Block, &sdh);
		if( bResult )
		{
...
void IE_Imp_RTF::_appendHdrFtr ()
{
	...
	for (i = 0; i < numHdrFtr; i++)
	{
		...
		{
			pf_Frag_Strux* sdh = getDoc()->getLastSectionMutableStrux();
