...
private:
	UT_GenericStringMap(const UT_GenericStringMap<T>&) = delete;
	void operator=(const UT_GenericStringMap<T>&) = delete;


	enum SM_search_type: uint8_t
	{
		SM_INSERT,
		SM_LOOKUP,
		SM_REORG
...
public:
	...
	void insert(const T v, const UT_String &k, UT_uint32 h)
		{
			m_value = v;
			// We don't want to invalidate the key if it is already
			// there, like when replacing an existing value.
			if (h != m_key.hashval() || m_key.value() != k) {
				m_key = k;
				m_key.set_hashval(h);
			}
		}
