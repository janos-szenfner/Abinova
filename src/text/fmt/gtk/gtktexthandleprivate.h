...
/* GTK4 port: the text handles are now plain widgets added to a
 * GtkOverlay over the drawing area, instead of child GdkWindows
 * (which GTK4 no longer has).  Coordinates passed to
 * _fv_text_handle_set_position() are in overlay coordinates, which
 * coincide with the drawing area coordinates.
 */
...
FvTextHandle *  _fv_text_handle_new          (GtkWidget             *overlay);
