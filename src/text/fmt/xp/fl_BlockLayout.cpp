...
bool
fl_BlockLayout::_spellCheckWord(const UT_UCS4Char * word,
...
void fl_BlockLayout::updateEnclosingBlockIfNeeded(void)
{
	...
	const pf_Frag_Strux* sdhStart = pCL->getStruxDocHandle();
...
bool fl_BlockLayout::setFramesOnPage(fp_Line * pLastLine)
{
	...
	for(i=0; i< getNumFrames();i++)
	{
		const fl_FrameLayout * pFrame = getNthFrameLayout(i);
		...
		else if(pFrame->getFramePositionTo() == FL_FRAME_POSITIONED_TO_COLUMN)
		{
			...
			if(pFrameCon)
			{
				...
				{
					const fp_Page * pPageFinal = pDL->getLastPage();
					if (!pPageLast && (pDL->findPage(pPageFinal) <= iPrefPage)) 
					{
						if (pDL->findPage(pPageFinal) == iPrefPage)
						{
							...
							for(j=0;j<pPageFinal->countColumnLeaders() && !b_sectionFound;j++)
							{
								...
								{
									b_sectionFound = true;
									const fp_Container * pCol2 = pPageFinal->getNthColumnLeader(j);
									while(k < iPrefColumn && pCol2)
									{
										pCol2 = static_cast<const fp_Container *>(pCol2->getNext());
										k++;
									}
								}
							}
				...
				else
				{
					...
					if(pPageFirst != pPageLast)
					{
						...
						{
							pPage = pPageFirst;
							auto firstColumn = static_cast<const fp_Column *>(pLFirst->getColumn());
						...
						{
							pPage = pPageLast;
							auto lastColumn = static_cast<const fp_Column *>(pLLast->getColumn());
				...
				{
					auto pColumn = static_cast<const fp_Column *>(pCol);
		...
		else if(pFrame->getFramePositionTo() == FL_FRAME_POSITIONED_TO_PAGE)
		{
			...
			if(pFrameCon)
			{
				...
				//
				UT_sint32 iPrefPage = pFrameCon->getPreferedPageNo();
				auto pLFirst = static_cast<const fp_Line *>(getFirstContainer());
				UT_return_val_if_fail(pLFirst,false);
				auto pLLast = static_cast<const fp_Line *>(getLastContainer());
				UT_return_val_if_fail(pLLast,false);
				fp_Page * pPageFirst = pLFirst->getPage();
				UT_return_val_if_fail(pPageFirst,false);
				fp_Page * pPageLast = pLLast->getPage();
...
bool fl_BlockLayout::getXYOffsetToLine(UT_sint32 & xoff, UT_sint32 & yoff, fp_Line * pLine) const
{
	...
	xoff = 0;
	yoff = 0;
	auto pCon = static_cast<const fp_Line *>(getFirstContainer());
...
UT_sint32 fl_BlockLayout::getHeightOfBlock(bool b_withMargins) const
{
	...
	while(pCon)
	{
		...
		pCon = static_cast<const fp_Line *>(pCon->getNext());
	}
	return iHeight;
}
											  ...
											  UT_sint32 & iMinWidth)
{
	...
	for(i=0; i< static_cast<UT_sint32>(pPage->countAboveFrameContainers());i++)
	{
		...
		const fl_FrameLayout * pFL = static_cast<fl_FrameLayout *>(pFC->getSectionLayout());
		...
		if(iMinR + xoff - iMinLeft > getMinWrapWidth())
	   	{
			...
			const fp_FrameContainer * pRightC = nullptr;
			for(i=0; i< static_cast<UT_sint32>(pPage->countAboveFrameContainers());i++)
			{
				projRec.left = iScreenX;
				projRec.height = iHeight;
				...
				bIsTight = pFC->isTightWrapped();
				UT_Rect pRec = pFC->getScreenRect().value();
				auto pFL = static_cast<const fl_FrameLayout *>(pFC->getSectionLayout());
				iExpand = pFL->getBoundingSpace() + 2;
				pRec.height += 2 * iExpand;
				pRec.width += 2 * iExpand;
				pRec.left -= iExpand;
...
void fl_BlockLayout::format()
{
	...
	// branch for now
#if 1
	const fp_Container * pPrevCon = getFirstContainer();
	if(pPrevCon)
	{
	...
	if (m_pFirstRun)
	{
		...
		while (pRun)
		{
			...
			{
				auto pHRun = static_cast<const fp_HyperlinkRun *>(pRun);
	...
	if((m_bForceSectionBreak) || (iOldHeight != iNewHeight))
	{
		...
		if(getSectionLayout()->getContainerType() != FL_CONTAINER_DOCSECTION)
		{
			...
			if(getSectionLayout()->getContainerType() == FL_CONTAINER_CELL)
			{
				...
				// the cell.
				//
				auto pCL = static_cast<const fl_CellLayout *>(getSectionLayout());
...
{
	auto pTmpLine = static_cast<const fp_Line *>(getFirstContainer());
	UT_sint32 i = 0;
	while(pTmpLine && pTmpLine != pLine)
	{
		i++;
		pTmpLine = static_cast<const fp_Line *>(pTmpLine->getNext());
	}
	if(pTmpLine == nullptr)
	{
		return -1;
...
fl_BlockLayout::_recalcPendingWord(UT_uint32 iOffset, UT_sint32 chg) const
{
	...
	const UT_UCS4Char* pBlockText = reinterpret_cast<UT_UCS4Char*>(pgb.getPointer(0));
	...
	if (chg > 0)
	{
		...
		UT_UCS4Char followChar = UCS_UNKPUNK, currentChar, prevChar = iLast > 2 ? pBlockText[iLast - 2] : UCS_UNKPUNK;
		while (iLast > iFirst)
		{
	...
	{
		UT_UCS4Char currentChar = pBlockText[iFirst];
		UT_UCS4Char followChar = (((iFirst + 1) < eor) ?
								 pBlockText[iFirst + + 1]  : UCS_UNKPUNK);
		UT_UCS4Char prevChar = iFirst > 0 ? pBlockText[iFirst - 1] : UCS_UNKPUNK;
...
bool fl_BlockLayout::checkSpelling(void)
{
	...
	const FV_View* pView = getView();
	const fp_Run* pLastRun = m_pFirstRun;
...
void fl_BlockLayout::shuffleEmbeddedIfNeeded(fl_BlockLayout * pBlock, UT_uint32 blockOffset)
{
	...
	while(!bStop)
	{
		...
		//
		const pf_Frag_Strux* sdhStart = pEmbedCL->getStruxDocHandle();
		const pf_Frag_Strux* sdhEnd = nullptr;
...
*/
void	fl_BlockLayout::StartList( const gchar * style, const pf_Frag_Strux* prevSDH)
{
	//
...
const UT_UCS4Char * fl_BlockLayout::getListLabel(void) const
{
	//	UT_ASSERT(m_pAutoNum);
	//
...
*/
bool
fl_BlockSpellIterator::nextWordForSpellChecking(const UT_UCS4Char*& pWord, UT_sint32& iLength,
												UT_sint32& iBlockPos, UT_sint32& iPTLength)
{
	...
	for (;;) {
		...
		if (0 == m_iWordOffset)
		{
			UT_UCS4Char followChar = (((m_iWordOffset + 1) < m_iLength)  
		...
		{
			UT_UCS4Char currentChar = m_pText[m_iWordOffset + i];
		...
		if (bNeedsMutation || bRevised)
		{
			...
			m_pMutatedString = static_cast<UT_UCS4Char*>(UT_calloc(iWordLength, sizeof(UT_UCS4Char)));
...
fl_BlockSpellIterator::getPreWord(UT_sint32& iLength) const
{
	...
	return reinterpret_cast<UT_UCS4Char*>(m_pgb->getPointer(m_iSentenceStart));
}
...
fl_BlockSpellIterator::getPostWord(UT_sint32& iLength) const
{
	...
	return reinterpret_cast<UT_UCS4Char*>(m_pgb->getPointer(m_iStartIndex));
}
...
bool
fl_BlockSpellIterator::_ignoreFirstWordCharacter(const UT_UCS4Char c) const
{
    switch (c) {
    case '\'':
    case '"':
...
bool
fl_BlockSpellIterator::_ignoreLastWordCharacter(const UT_UCS4Char c) const
{
    switch (c) {
    case '\'':
    case '"':
