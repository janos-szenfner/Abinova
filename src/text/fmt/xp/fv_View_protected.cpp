...
bool FV_View::_isSpaceBefore(PT_DocPosition pos) const
{
	...
	if (block)
	{
		...
		if (offset > 0)
		{
			...
			return (UT_UCS4_isspace(*reinterpret_cast<UT_UCS4Char *>(buffer.getPointer(offset - 1))));
		}
		else
		{
			return true;
...
//
	fl_FootnoteLayout * pFHigh = nullptr;
	fl_FootnoteLayout * pFLow = nullptr;
	fl_EndnoteLayout * pEHigh = nullptr;
	fl_EndnoteLayout * pELow = nullptr;
...
//
	...
	//
	pBL = getCurrentBlock();
	if(pBL && (pBL->getNextBlockInDocument() == nullptr) && (pBL->getPrevBlockInDocument() == nullptr))
	{
...
//
	...
	const pf_Frag_Strux* sourceSDH, *destinationSDH;
	bool bres = m_pDoc->getStruxOfTypeFromPosition(posSource,PTX_SectionCell,&sourceSDH);
	if(!bres)
	{
		return false;
	}
	const pf_Frag_Strux* endSourceStrux =  m_pDoc->getEndCellStruxFromCellStrux(sourceSDH);
	PT_DocPosition posEndCell = m_pDoc->getStruxPosition(endSourceStrux) - 1;
	...
	const pf_Frag_Strux* endDestStrux = m_pDoc->getEndCellStruxFromCellStrux(destinationSDH);
	PT_DocPosition posEndDestCell = m_pDoc->getStruxPosition(endDestStrux);
//	if(!bBefore)
//	{
 ...
 */
bool FV_View::_restoreCellParams(PT_DocPosition posTable, const pf_Frag_Strux * tableSDH)
{
	//
 ...
 */
bool FV_View::_changeCellParams(PT_DocPosition posTable, const pf_Frag_Strux* tableSDH)
{
...
bool FV_View::_deleteCellAt(PT_DocPosition posTable, UT_sint32 row, UT_sint32 col)
{
	...
	PT_DocPosition posEndCell = m_pDoc->getStruxPosition(endCellStrux) + 1;
	if(posEndCell == 0)
	{
		return false;
	}
...
//
	...
	m_pDoc->deleteSpan( posCell, posEndCell, nullptr,iRealDeleteCount,true);
...
// TODO
	...
	case FV_DOCPOS_BOW:
	{
		...
		const UT_UCS4Char* pSpan = reinterpret_cast<UT_UCS4Char*>(pgb.getPointer(0));
		...
		if (offset == 0)
		{
			...
			pSpan = reinterpret_cast<UT_UCS4Char*>(pgb.getPointer(0));
...
/*!
  ...
  \return Block at specified posistion, or the first block to the
		  rigth of that position. May return nullptr.
...
fl_BlockLayout* FV_View::_findBlockAtPosition(PT_DocPosition pos) const
{
	...
	if(m_bEditHdrFtr && (m_pEditShadow != nullptr) && (m_FrameEdit.getFrameEditMode() == FV_FrameEdit_NOT_ACTIVE))
	{
...
{
	fp_ContainerObject * pNext = nullptr;
	fl_ContainerLayout * pCL = nullptr;
	fl_BlockLayout * pNextB = nullptr;
...
void FV_View::_moveInsPtNextPrevScreen(bool bMovingDown, bool bClearSelection)
{
	...
	if (pPage == nullptr) pPage = pLine->getPage ();
...
*/
UT_uint32*
FV_View::_computeFindPrefix(const UT_UCS4Char* pFind)
{
								 ...
								 PT_DocPosition* pOffset)
{
	...
	fl_BlockLayout* newBlock = nullptr;
...
{
	endIndex = 0;
	UT_return_val_if_fail(m_pLayout && pBlock && *pBlock && pOffset,nullptr);
...
// Any takers?
UT_sint32
FV_View::_findBlockSearchRegexp(const UT_UCS4Char* /* haystack */,
								const UT_UCS4Char* /* needle */)
{
...
//
	...
	if(getSelectionAnchor() < getPoint())
	{
		...
		fp_CellContainer * pLowCell = nullptr;
		fp_CellContainer * pHighCell = nullptr;
		if(isInTable(posLow) )
		{
			pLowCell = getCellAtPos(posLow+1);
			pHighCell =  getCellAtPos(getPoint());
			if((pLowCell != nullptr) && (pLowCell != pHighCell))
			{
...
//
	...
	// dereferencing nullptr.	Dom 11.9.00
	if(!pBlock)
	{
...
//
	...
	else if( (pRun == nullptr) && (posEOD == getPoint()))
	{
...
{
	if (m_pG->allCarets()->getBaseCaret() == nullptr)
		return;
	if(!bIgnoreAll)
					...
					bool bDirtyRunsOnly, bool bClip)
{
	...
	if(m_pViewDoubleBufferingObject != nullptr && m_pViewDoubleBufferingObject->getCallDrawOnlyAtTheEnd())
	{
...
// _findPositionCoords signals this by returning pRun == nullptr
//
		...
		bool bExtra = false;
		while(m_iInsPoint <= posEOD && (pRun == nullptr /*|| ((x == xold) && (y == yold) &&
														 (x2 == x2old) && (y2 == y2old) &&
...
//
		...
		bool bExtra = false;
		while( m_iInsPoint >= posBOD && (pRun == nullptr /*|| ((x == xold) && (y == yold) &&
														 (x2 == x2old) && (y2 == y2old) &&
														 (bDirection == bDirectionOld))*/))
		{
...
UT_Error FV_View::_deleteBookmark(const char* szName, bool bSignal, PT_DocPosition *posStart, PT_DocPosition *posEnd)
{
	if(!m_pDoc->isBookmarkUnique(static_cast<const gchar *>(szName)))
	{
		...
		m_pDoc->deleteSpan(pos1,pos1 + 1,nullptr,iRealDeleteCount);
												...
												PT_DocPosition &posEnd)
{
	...
	// Now, getHyperlink() looks nullptr, so let's step forward till posEnd.
	...
	{
		pRun = pRun->getNextRun();
		if (pRun && pRun->getPrevRun() && pRun->getPrevRun()->getHyperlink() != nullptr)
...
UT_Error FV_View::_deleteHyperlink(PT_DocPosition &pos1, bool bSignal)
{
	...
	UT_uint32 iRealDeleteCount;
	m_pDoc->beginUserAtomicGlob();
	m_pDoc->deleteSpan(pos1,pos1 + iRunLen,nullptr, iRealDeleteCount);
...
UT_UCS4Char * FV_View::_lookupSuggestion(fl_BlockLayout* pBL,
										const fl_PartOfBlockPtr& pPOB, UT_sint32 ndx)
{
	// mega caching - are these assumptions valid?
	UT_UCS4Char * szSuggest = nullptr;
...
void FV_View::_prefsListener( XAP_Prefs *pPrefs, const XAP_PrefsChangeSet* /*phChanges*/, void *data )
{
...
//
	const gchar * pszHdrFtrType = nullptr;
	UT_ASSERT(pHdrFtr->getContainerType() == FL_CONTAINER_HDRFTR);
	pf_Frag_Strux* sdhHdrFtr = pHdrFtr->getStruxDocHandle();
	m_pDoc->getAttributeFromStrux(sdhHdrFtr, isShowRevisions(), getRevisionLevel(), PT_TYPE_ATTRIBUTE_NAME, &pszHdrFtrType);
//
