 ...
 */
UT_uint32 fl_TableLayout::getLength(void) const
{
	const pf_Frag_Strux* sdhTab = getStruxDocHandle();
	const pf_Frag_Strux* sdhEnd = m_pDoc-> getEndTableStruxFromTableStrux(sdhTab);
...
bool fl_CellLayout::isCellSelected(void)
{
	...
	const pf_Frag_Strux* sdhEnd, *sdhStart;
...
{
	const pf_Frag_Strux* sdhCell = getStruxDocHandle();
	const pf_Frag_Strux* sdhEnd = m_pDoc->getEndCellStruxFromCellStrux(sdhCell);
	PT_DocPosition posEnd = 0;
	PT_DocPosition posStart = 0;
	UT_uint32 len = 0;
