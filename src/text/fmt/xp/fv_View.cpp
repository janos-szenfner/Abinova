...
static void _toggleLower (const UT_UCS4Char * src,
...
static void _toggleUpper (const UT_UCS4Char * src,
			  UT_UCS4Char * dest, UT_uint32 len)
{
	for (UT_uint32 i = 0; i < len; i++)
	{
...
static void _toggleToggle (const UT_UCS4Char * src,
			   UT_UCS4Char * dest, UT_uint32 len)
{
	UT_UCS4Char ch;
	for (UT_uint32 i = 0; i < len; i++)
...
//
	...
	UT_UCS4Char * pTemp = new UT_UCS4Char[iTempLen];
	UT_ASSERT(pTemp);
	...
	while(pBL && low < high)
	{
		...
		if(c == CASE_ROTATE)
		{
			...
			UT_UCS4Char * pT = reinterpret_cast<UT_UCS4Char*>(buffer.getPointer(offset));
		...
		while(!bBlockDone && (low < high) /*&& (low < lastPos)*/)
		{
			...
			{
				delete[] pTemp;
				pTemp = new UT_UCS4Char[iLenToCopy];
				iTempLen = iLenToCopy;
				UT_ASSERT(pTemp);
			}
			...
			while(pRun && iLenToCopy > 0)
			{
				...
				memmove(pTemp, buffer.getPointer(offset), iLen * sizeof(UT_UCS4Char));
				...
				switch (c)
				{
					case CASE_FIRST_CAPITAL:
						if(offset == 0)
							...
							prev = reinterpret_cast<UT_UCS4Char*>(buffer.getPointer(offset - 1));
 ...
 */
bool FV_View::getLineBounds(PT_DocPosition pos, PT_DocPosition *start, PT_DocPosition *end) const
{
...
UT_UCS4Char FV_View::getChar(PT_DocPosition pos, UT_sint32 *x, UT_sint32 *y, UT_uint32 *width, UT_uint32 *height) const
{
  if (x || y || height)
  {
  UT_sint32 fp_x, fp_y;
  ...
  const pt_PieceTable *piece = getDocument()->getPieceTable();
  const pf_Frag *p;
  ...
  if (piece->getFragFromPosition(pos, &p, &offset))
  {
    ...
    {
      auto pt = static_cast<const pf_Frag_Text *>(p);
      PT_BufIndex bi = pt->getBufIndex();
      const UT_UCS4Char *c = piece->getPointer(bi);
      ret = c[offset];
    }
  }
...
PT_DocPosition FV_View::mapDocPos( FV_DocPos dp ) const {
	return ( _getDocPos( dp ));
	}

PT_DocPosition FV_View::mapDocPosSimple( FV_DocPos dp ) const {
...
//
		bool bAttach = false;
		const pf_Frag_Strux* prevSDH = m_pDoc->getPrevNumberedHeadingStyle(curSdh);
		if(prevSDH != nullptr)
		{
			PD_Style * pPrevStyle = m_pDoc->getStyleFromStrux(prevSDH);
...
//
					...
					{
						const pf_Frag_Strux* subSDH = pAuto->getFirstItem();
...
//
			...
			const pf_Frag_Strux* sdh = m_pDoc->findPreviousStyleStrux(style, pos);
 ...
 */
fl_BlockLayout * FV_View::getBlockFromStrux(const pf_Frag_Strux* sdh)
{
...
bool FV_View::isImageAtStrux(PT_DocPosition ipos1, PTStruxType iStrux)
{
    ...
    bret = m_pDoc->getAttributeFromStrux(sdh, isShowRevisions(), getRevisionLevel(), PT_STRUX_IMAGE_DATAID,&pszDataID);
	if(!bret)
	{
		return false;
	}
...
void FV_View::getSelectionText(UT_UCS4Char * & pText) const
{
	...
	if (block)
	{
		...
		bufferSegment = static_cast<UT_UCS4Char *>(UT_calloc(selLength + 1, sizeof(UT_UCS4Char)));
		...
		// copy it out
		memmove(bufferSegment, buffer.getPointer(offset), selLength * sizeof(UT_UCS4Char));
 ...
 */
UT_UCS4Char * FV_View::getTextBetweenPos(PT_DocPosition pos1, PT_DocPosition pos2) const
{
	...
	UT_UCS4Char * bufferRet = new UT_UCS4Char[iLength+1];

	UT_ASSERT(bufferRet);
	if(!bufferRet)
		return nullptr;

	UT_UCS4Char * buff_ptr = bufferRet;
	...
	while(pBlock && curPos < pos2)
	{
		...
		{
			memmove(buff_ptr, buffer.getPointer(offset), iLenToCopy * sizeof(UT_UCS4Char));
			offset	 += iLenToCopy;
			buff_ptr += iLenToCopy;
			curPos	 += iLenToCopy;
			if (curPos < pos2)
...
// ---------------- start goto ---------------
bool FV_View::gotoTarget(AP_JumpTarget type, const UT_UCS4Char *data)
{
...
*/
bool
FV_View::findNext(const UT_UCS4Char* pFind, bool& bDoneEntireDocument)
...
bool
FV_View::findPrev(const UT_UCS4Char* pFind, bool& bDoneEntireDocument)
{
	findSetFindString(pFind);
...
void
FV_View::findSetFindString(const UT_UCS4Char* pFind)
{
	FREEP(m_sFind);
...
void
FV_View::findSetReplaceString(const UT_UCS4Char* pReplace)
{
	FREEP(m_sReplace);
...
void FV_View::insertSymbol(UT_UCS4Char c, const gchar * symfont)
{
							 ...
							 UT_sint32 * pTop, UT_sint32 * pBot) const
{
	...
	const char * pszTop;
	const char * pszBot;
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"left-attach",&pszLeft);
	if(pszLeft && *pszLeft)
	{
		*pLeft = atoi(pszLeft);
	}
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"right-attach",&pszRight);
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"top-attach",&pszTop);
	if(pszTop && *pszTop)
	{
		*pTop = atoi(pszTop);
	}
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"bot-attach",&pszBot);
	if(pszBot && *pszBot)
	{
		*pBot = atoi(pszBot);
	}
							 ...
							 UT_sint32 * pTop, UT_sint32 * pBot) const
{
	...
	const char * pszTop;
	const char * pszBot;
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"left-style",&pszLeft);
	if(pszLeft && *pszLeft)
	{
		*pLeft = atoi(pszLeft);
	}
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"right-style",&pszRight);
	if(pszRight && *pszRight)
	{
		*pRight = atoi(pszRight);
	}
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"top-style",&pszTop);
	if(pszTop && *pszTop)
	{
		*pTop = atoi(pszTop);
	}
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),"bottom-style",&pszBot);
	if(pszBot && *pszBot)
	{
		*pBot = atoi(pszBot);
	}
...
bool FV_View::setCellFormat(const PP_PropertyVector & properties, FormatTable applyTo, const FG_ConstGraphicPtr & pFG, const std::string & sDataID)
{
	...
	if (applyTo == FORMAT_TABLE_SELECTION)
	{
		...
		if(bRet)
		{
			...
			//
			const pf_Frag_Strux* endTableStrux = m_pDoc->getEndTableStruxFromTablePos(posTable);
			UT_ASSERT(endTableStrux);
	...
	else if(applyTo == FORMAT_TABLE_TABLE)
	{
		...
		UT_sint32 numRows;
		UT_sint32 numCols;
		bRet = m_pDoc->getRowsColsFromTableStrux(tableSDH, isShowRevisions(), getRevisionLevel(), &numRows, &numCols);
		...
		for (j = rowStart; j <= rowEnd; j++)
		{
			...
			{
				const pf_Frag_Strux* cellStrux = m_pDoc->getCellStruxFromRowCol(tableSDH, isShowRevisions(), getRevisionLevel(),
																		 j, i);
				if (cellStrux)
				{
					// Do the actual change
					posStart = m_pDoc->getStruxPosition(cellStrux) + 1;
...
bool FV_View::getCellProperty(PT_DocPosition posCell, const gchar * szPropName, gchar * &szPropValue) const
{
	...
	m_pDoc->getPropertyFromStrux(cellSDH,isShowRevisions(),getRevisionLevel(),szPropName,const_cast<const char **>(&szPropValue));
...
UT_UCS4Char * FV_View::getContextSuggest(UT_uint32 ndx)
{
	// locate the squiggle
	PT_DocPosition pos = 0 ;
...
static bool s_notChar(UT_UCS4Char c)
{
    switch (c)
    {
    case UCS_TAB:
...
*/
FV_DocCount FV_View::countWords(bool bActuallyCountWords) const
{
	FV_DocCount wCount;
	...
	while (pBL)
	{
		...
		UT_GrowBuf gb(1024);
		pBL->getBlockBuf(&gb);
		const UT_UCS4Char * pSpan = reinterpret_cast<UT_UCS4Char*>(gb.getPointer(0));
		...
		if (bActuallyCountWords)
		{
			...
			for (; i < imax; i++)
			{
				...
				UT_UCS4Char followChar, prevChar;
...
{
	const pf_Frag_Strux* prevSDH = nullptr;
...
bool FV_View::isInTable( PT_DocPosition pos) const
{
	...
	if(pCL->getContainerType() == FL_CONTAINER_TABLE)
	{
		...
		const pf_Frag_Strux* struxEnd = m_pDoc->getEndTableStruxFromTableStrux(sdh);
		if (struxEnd != nullptr)
		{
			PT_DocPosition posEnd = m_pDoc->getStruxPosition(struxEnd);
			if(posEnd == pos)
			{
...
//
	...
	const pf_Frag_Strux* cellStrux = m_pDoc->getCellStruxFromRowCol(tableSDH, isShowRevisions(), getRevisionLevel(), row, col);
	if (cellStrux == nullptr)
	{
		return 0;
	}
	return m_pDoc->getStruxPosition(cellStrux);
}
