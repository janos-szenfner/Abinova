...
FPVisibility fl_ContainerLayout::getAP(const PP_AttrProp *& pAP)const
{
	...
	getAttrProp(&pAP, unused, bShow, iId, bHiddenRevision);
...
void fl_ContainerLayout::getSpanAP(UT_uint32 blockPos, bool bLeft, const PP_AttrProp * &pSpanAP) const
{
	...
	std::optional<std::unique_ptr<PP_RevisionAttr>> unused;
	getSpanAttrProp(blockPos, bLeft, &pSpanAP, unused, bShow,iId,bHiddenRevision);
}


const char * fl_ContainerLayout::getContainerString(void) const
{
 ...
 */
UT_sint32 fl_ContainerLayout::getLevelInList(void) const
{
      const fl_BlockLayout * pBList = nullptr;
      if(getContainerType() == FL_CONTAINER_BLOCK)
      {
	   pBList = static_cast<const fl_BlockLayout * >(this);
      }
      else
      {
 ...
 */
UT_sint32 fl_ContainerLayout::getFoldedLevel(void) const
{
