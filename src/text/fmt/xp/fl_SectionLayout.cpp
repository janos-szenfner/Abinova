...
//
	...
	const pf_Frag_Strux* sdStart = getFirstLayout()->getStruxDocHandle();
	const pf_Frag_Strux* sdEnd = nullptr;
											  ...
											  fl_TableLayout * pTL)
{
	...
	for (UT_uint32 i=0; i<iCount; i++)
	{
		...
		auto shadow = pPair->getShadow();
		UT_nonnull_or_continue(shadow);
		pShadowBL = shadow->findMatchingContainer(pTL);
											  ...
											  PL_ListenerId lid)
{
	...
	for (UT_uint32 i=0; i<iCount; i++)
	{
		...
		auto shadow = pPair->getShadow();
		UT_nonnull_or_continue(shadow);
		pShadowBL = shadow->findMatchingContainer(pTab);
...
bool fl_HdrFtrSectionLayout::bl_doclistener_insertFirstBlock(fl_ContainerLayout* pCL, const PX_ChangeRecord_Strux * pcrx,pf_Frag_Strux* sdh,PL_ListenerId lid)
{
	...
	for (UT_uint32 i=0; i<iCount; i++)
	{
		...
		auto shadow = pPair->getShadow();
		UT_nonnull_or_continue(shadow);
		pShadowBL = shadow->findMatchingContainer(pCL);
