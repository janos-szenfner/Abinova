...
#define CONV_TO_UCS (UT_UCS4Char) (unsigned char)
...
void fl_AutoNum::fixHierarchy(void)
{
	...
	if (!m_items.empty())
	{
		const pf_Frag_Strux* pfs = m_items.front();
		XAP_Frame * pFrame = XAP_App::getApp()->getLastFocussedFrame();
		FV_View* pView = nullptr;
		if (pFrame)
			pView = static_cast<FV_View *>(pFrame->getCurrentView());

		bool bFound = m_pDoc->getAttributeFromStrux(pfs,
...
const UT_UCS4Char * fl_AutoNum::getLabel(const pf_Frag_Strux* pItem)  const
{
	...
	{
		return static_cast<const UT_UCS4Char *>(label);
	}
}
...
pf_Frag_Strux* fl_AutoNum::getLastItemInHierarchy(void) const
{
...
///
bool fl_AutoNum::isContainedByList(const pf_Frag_Strux* pItem) const
{
	const pf_Frag_Strux *sdh, *sdh_prev, *sdh_next;
