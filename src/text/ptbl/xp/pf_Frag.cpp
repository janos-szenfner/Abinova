...
pf_Frag::pf_Frag(pt_PieceTable * pPT, PFType type, UT_uint32 length, PT_AttrPropIndex indexAP):
	m_type(type),
	m_pField(nullptr),
	m_pPieceTable(pPT),
	m_indexAP(indexAP),
...
const pf_Frag_Strux* tryDownCastStrux(const pf_Frag* pf, PTStruxType t)
{
    if (!pf) {
        return nullptr;
    }
