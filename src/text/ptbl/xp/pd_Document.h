...
public:
	...
	AP_StatusBar *          getStatusBar(void) const;
	...
	ImagePage *             getNthImagePage(UT_sint32 iImagePage) const;
	TextboxPage *           getNthTextboxPage(UT_sint32 iTextboxPage) const;
 ...
 private:
	...
	bool                    _buildAuthorProps(pp_Author * pAuthor, PP_PropertyVector & props) const;
 public:
	//
...
PT_AttrPropIndex            getAPIFromSOH(pf_Frag_Object* odh) const;
	...
	bool                    createAndSendCR(PT_DocPosition dpos,UT_sint32 iType,bool bsave,UT_Byte iGlob) const;
	...
	bool                    createAndSendDocPropCR(const PP_PropertyVector & pAtts,const PP_PropertyVector & pProps) const;
	...
	bool					appendSpan(const UT_UCS4Char * p, UT_uint32 length);
	...
	bool					insertStruxBeforeFrag(const pf_Frag * pF, PTStruxType pts,
												  const PP_PropertyVector & attributes, pf_Frag_Strux ** ppfs_ret = nullptr);
	bool					insertSpanBeforeFrag(const pf_Frag * pF, const UT_UCS4Char * p, UT_uint32 length);
	bool					insertObjectBeforeFrag(const pf_Frag * pF, PTObjectType pto,
												   const PP_PropertyVector & attributes);
	bool					insertFmtMarkBeforeFrag(const pf_Frag * pF);
	bool					insertFmtMarkBeforeFrag(const pf_Frag * pF, const PP_PropertyVector & attributes);
	...
	bool					tellListener(PL_Listener * pListener) const;
	bool					tellListenerSubset( PL_Listener * pListener,
                                                PD_DocumentRange * pDocRange,
                                                PL_ListenerCoupleCloser* closer = nullptr) const;
	...
	bool					getSpanAttrProp(const pf_Frag_Strux* sdh, UT_uint32 offset, bool bLeftSide,
	...
	const UT_UCS4Char *		getPointer(PT_BufIndex bi) const; /* see warning on this function */
	bool					getBlockBuf(const pf_Frag_Strux* sdh, UT_GrowBuf * pgb) const;

	bool					getBounds(bool bEnd, PT_DocPosition & docPos) const;
	PTStruxType             getStruxType(const pf_Frag_Strux* sdh) const;
	PT_DocPosition			getStruxPosition(const pf_Frag_Strux* pfs) const;
	...
	bool					getStruxOfTypeFromPosition(PL_ListenerId listenerId,
													   ...
													   PTStruxType pts,
													   fl_ContainerLayout* * psfh) const;
	bool					getStruxOfTypeFromPosition(PT_DocPosition, PTStruxType pts, const pf_Frag_Strux* * sdh) const;
	/* This is used by the importers. */
	bool					getMutStruxOfTypeFromPosition(PT_DocPosition, PTStruxType pts, pf_Frag_Strux* * sdh) const;
    pf_Frag_Strux*       getBlockFromPosition( PT_DocPosition pos ) const;

	pf_Frag *				getFragFromPosition(PT_DocPosition docPos) const;

	bool					getNextStruxOfType(const pf_Frag_Strux* pfs, PTStruxType pts,
											   const pf_Frag_Strux* * nextsdh) const;
	bool					getPrevMutStruxOfType(const pf_Frag_Strux* pfs, PTStruxType pts,
											   pf_Frag_Strux* * prevsdh) const;
	bool					getPrevStruxOfType(const pf_Frag_Strux* pfs, PTStruxType pts,
											   const pf_Frag_Strux* * prevsdh) const;
	bool                    getNextMutStrux(const pf_Frag_Strux* sdh, pf_Frag_Strux* *next) const;
	bool                    getNextStrux(const pf_Frag_Strux* sdh, const pf_Frag_Strux* *next) const;
	...
	const pf_Frag_Strux*       getLastSectionStrux(void) const;
	pf_Frag_Strux*       getLastSectionMutableStrux(void) const;
	pf_Frag_Strux*       getLastStruxOfType(PTStruxType pts) const;
	...
	bool                    insertStruxNoUpdateBefore(const pf_Frag_Strux* sdh, PTStruxType pts, const PP_PropertyVector & attributes );
	bool                    isStruxBeforeThis(const pf_Frag_Strux* sdh,  PTStruxType pts) const;
	...
	PT_AttrPropIndex        getAPIFromStrux(const pf_Frag_Strux* sdh) const;
    bool                    getAttributeFromStrux(const pf_Frag_Strux* sdh, bool bShowRevisions, UT_uint32 iRevisionLevel,
												const char * szAttribute, const char ** pszValue) const;
	...
	PD_Style *				getStyleFromStrux(const pf_Frag_Strux* pfs) const;
	const pf_Frag_Strux*    getPrevNumberedHeadingStyle(const pf_Frag_Strux* pfs) const;
    ...
    const pf_Frag_Strux*       findPreviousStyleStrux(const gchar * szStyle, PT_DocPosition pos) const;
	...
	po_Bookmark * 			getBookmark(const pf_Frag_Strux* sdh, UT_uint32 offset) const;
	...
	// Footnote functions
	bool                    isFootnoteAtPos(PT_DocPosition pos) const;
	bool                    isEndFootnoteAtPos(PT_DocPosition pos) const;
	UT_sint32               getEmbeddedOffset(const pf_Frag_Strux* sdh,PT_DocPosition posOff, pf_Frag_Strux* & sdhEmbedded) const;
	bool                    hasEmbedStruxOfTypeInRange(PT_DocPosition posStart, PT_DocPosition posEnd, 
													   PTStruxType iType) const;

	// TOC functions
	bool                    isTOCAtPos(PT_DocPosition pos) const;

	// FRAME function
	bool                    isFrameAtPos(PT_DocPosition pos) const;
	bool                    isEndFrameAtPos(PT_DocPosition pos) const;
	bool                    isHdrFtrAtPos(PT_DocPosition pos) const;
	bool                    isSectionAtPos(PT_DocPosition pos) const;
	bool                    isBlockAtPos(PT_DocPosition pos) const;

// Table functions

	bool                    isTableAtPos(PT_DocPosition pos) const;
	bool                    isEndTableAtPos(PT_DocPosition pos) const;
	bool                    isCellAtPos(PT_DocPosition pos) const;
	pf_Frag_Strux*    getEndTableMutStruxFromTableStrux(const pf_Frag_Strux* table) const;
	const pf_Frag_Strux*    getEndTableStruxFromTableStrux(const pf_Frag_Strux* table) const;
	pf_Frag_Strux*    getEndCellMutStruxFromCellStrux(const pf_Frag_Strux* cell) const;
	const pf_Frag_Strux*    getEndCellStruxFromCellStrux(const pf_Frag_Strux* cell) const;
	const pf_Frag_Strux*       getEndTableStruxFromTablePos(PT_DocPosition posTable) const;
	bool                 getRowsColsFromTableStrux(const pf_Frag_Strux* table,
													bool bShowRevisions, UT_uint32 iRevisionLevel,
													UT_sint32 * numRows, UT_sint32 * numCols) const;
	pf_Frag_Strux*    getCellMutStruxFromRowCol(const pf_Frag_Strux* table,
												 bool bShowRevisions, UT_uint32 iRevisionLevel,
												 UT_sint32 row,
												 UT_sint32 col) const;
	const pf_Frag_Strux*    getCellStruxFromRowCol(const pf_Frag_Strux* table,
												 bool bShowRevisions, UT_uint32 iRevisionLevel,
												 UT_sint32 row,
												 UT_sint32 col) const;
	void                    miniDump(const pf_Frag_Strux* sdh, UT_sint32 nstruxes) const;
	...
	bool					enumLists(UT_uint32 k, fl_AutoNumConstPtr & pAutoNum) const;
	...
	void					removeList(const fl_AutoNumPtr & pAutoNum, const pf_Frag_Strux* sdh);
	void					listUpdate(const pf_Frag_Strux* pfs) const;
	void					StopList(const pf_Frag_Strux* pfs);
	void					disableListUpdates(void);
	void					enableListUpdates(void);
	void					updateDirtyLists(void);
	bool					areListUpdatesAllowed(void) const;
	...
	// PageSize functions
	bool                    convertPercentToInches(const char * szPercent, UT_UTF8String & sInches) const;
