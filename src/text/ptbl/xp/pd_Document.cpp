 ...
 */
bool PD_Document::_sendAuthorCR(const char *attrName, pp_Author *pAuthor) const
{
	const PP_PropertyVector atts = {
...
bool PD_Document::_buildAuthorProps(pp_Author * pAuthor, PP_PropertyVector & props) const
{
								  ...
								  bool bIsImportFile, const char* impProps)
{
	...
	if (pFrame && szFilename && (strstr(szFilename, "normal.awt") == nullptr)) {
		auto prefs = XAP_App::getApp()->getPrefs();
		prefs->addRecent(szFilename);
		prefs->savePrefsFile();
	}
...
AP_StatusBar *  PD_Document::getStatusBar(void) const
{
	XAP_Frame * pFrame = XAP_App::getApp()->getLastFocussedFrame();
	if(pFrame)
	{
 ...
 */
bool PD_Document::insertSpan(PT_DocPosition dpos, const UT_UCS4Char * pbuf,
...
/*!
 * This method deletes the HdrFtr strux pointed to by pfs_hdrftr
 */
void PD_Document::deleteHdrFtrStrux(pf_Frag_Strux* pfs_hdrftr)
{
 ...
 */
bool PD_Document::changeStruxForLists(pf_Frag_Strux* pfs, const char * pszParentID)
{
	return m_pPieceTable->changeStruxForLists(pfs, pszParentID);
}
...
bool PD_Document::_removeRepeatedHdrFtr(pf_Frag_Strux * pfs ,UT_GenericVector<pf_Frag_Strux *> * vecHdrFtrs,UT_sint32 iStart)
{
	...
	getAttributeFromStrux(pfs, false, 0, "type", &pszMyHdrFtr);
...
bool PD_Document::_pruneSectionAPI(pf_Frag_Strux * pfs,const char * szHType, UT_GenericVector<pf_Frag_Strux *> *vecHdrFtrs)
{
	...
	UT_sint32 i = 0;
	getAttributeFromStrux(pfs,false,0,szHType,&pszID);
	if(!pszID)
		return false;
	if(!(*pszID))
		return false;
	for(i= 0; i< vecHdrFtrs->getItemCount(); i++)
	{
		...
		if(pszHdrFtr && *pszHdrFtr)
		{
			...
			{
				getAttributeFromStrux(pfsS,false,0,"id",&pszHFID);
				if(pszHFID && *pszHFID)
				{
...
bool PD_Document::_matchSection(pf_Frag_Strux * pfs, UT_GenericVector<pf_Frag_Strux *> *vecSections) const
{
	...
	const char * pszID = nullptr;
	UT_sint32 i = 0;
	getAttributeFromStrux(pfs,false,0,"type",&pszHdrFtr);
	if(!pszHdrFtr)
		return false;
	if(!(*pszHdrFtr))
		return false;
	getAttributeFromStrux(pfs,false,0,"id",&pszHFID);
	if(!pszHFID)
		return false;
	if(!(*pszHFID))
		return false;
...
bool PD_Document::appendSpan(const UT_UCS4Char * pbuf, UT_uint32 length)
{
	UT_return_val_if_fail (m_pPieceTable, false);
	checkForSuspect();
 ...
 * at piecetable strux given by pfStrux.
 * NB: attributes and props are view-specific because of revision attributes
 * 
 \param  pf_Frag_Strux* pfStrux where we want to find the value
...
*/
bool PD_Document::getAttributeFromStrux(const pf_Frag_Strux* pfStrux, bool bShowRevisions, UT_uint32 iRevisionLevel,
									  const char * szAttribute, const char ** pszRetValue) const
{
 ...
 *     and attrs using getPropertyFromStrux() and getAttributeFromStrux().
 */
PT_AttrPropIndex PD_Document::getAPIFromStrux(const pf_Frag_Strux* pfStrux) const
{
 ...
 * 
 ...
 Don't FREEP *pszRetValue!!!
*/
bool PD_Document::getPropertyFromStrux(const pf_Frag_Strux* pfStrux, bool bShowRevisions, UT_uint32 iRevisionLevel,
 ...
 */
bool  PD_Document::changeStruxAttsNoUpdate(pf_Frag_Strux* pfStrux, const char * attr, const char * attvalue)
{
 ...
 */
bool PD_Document::createAndSendCR(PT_DocPosition dpos, UT_sint32 iType,bool bSave,UT_Byte iGlob) const
{
 ...
 */
bool PD_Document::createAndSendDocPropCR( const PP_PropertyVector & pAtts, const PP_PropertyVector & pProps ) const
{
 ...
 * requested.
 * if bRecordChange is false no change record is recorded.
							  ...
							  bool bRecordChange)
{
	...
	pf_Frag * pf = nullptr;
	m_pPieceTable->getMutFragFromPosition(dpos, &pf, &pOffset);
	...
	if(!bRecordChange)
	{
		return m_pPieceTable->deleteStruxNoUpdate(pfs);
	}
	if(getStruxPosition(pfs) != dpos)
		return false;
	return m_pPieceTable->deleteStruxWithNotify(pfs);
}
...
bool   PD_Document::isInsertHyperLinkValid(PT_DocPosition pos) const
{
	...
	while(pf && (pf->getType() != pf_Frag::PFT_Strux) )
	{
		...
		{
			auto pfo = static_cast<const pf_Frag_Object *>(pf);
 ...
 */
const pf_Frag_Strux*  PD_Document::getLastSectionStrux(void) const
{
 ...
 * This method returns the last pf_Frag_Strux as a pf_Frag_Strux* before the end of the piecetable.
 */
pf_Frag_Strux*  PD_Document::getLastSectionMutableStrux(void) const
{
 ...
 */
pf_Frag_Strux*  PD_Document::getLastStruxOfType(PTStruxType pts) const
{
 ...
 */ 
UT_sint32 PD_Document::getEmbeddedOffset(const pf_Frag_Strux* pfs, PT_DocPosition posoff, pf_Frag_Strux* & sdhEmbedded) const
{
	UT_return_val_if_fail (pfs->getStruxType() == PTX_Block,-1);
	pf_Frag * pf = pfs->getNext();
	PT_DocPosition pos = m_pPieceTable->getStruxPosition(pfs) + posoff;
 ...
 * position.
 */
bool PD_Document::isFootnoteAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isTOCAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
	...
	if (pf && (pf->getType() == pf_Frag::PFT_Strux)) {
		auto pfs = static_cast<const pf_Frag_Strux *>(pf);
		if (pfs->getStruxType() == PTX_SectionTOC || pfs->getStruxType() == PTX_EndTOC)	{
			return true;
		}
	}
 ...
 * position.
 */
bool PD_Document::isEndFootnoteAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
 ...
 * position.
 */
bool PD_Document::isFrameAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isEndFrameAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isHdrFtrAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isSectionAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isBlockAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isTableAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isEndTableAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
 ...
 * position.
 */
bool PD_Document::isCellAtPos(PT_DocPosition pos) const
{
	PT_BlockOffset pOffset;
	const pf_Frag * pf = nullptr;
	/*bool bRes = */m_pPieceTable->getFragFromPosition(pos,&pf,&pOffset);
...
/*!
 * This method returns the end table strux associated with the table strux tableStrux
 * Returns nullptr on failure to find it.
 */
const pf_Frag_Strux* PD_Document::getEndTableStruxFromTableStrux(const pf_Frag_Strux* table) const
{
	return getEndTableMutStruxFromTableStrux(table);
}

pf_Frag_Strux* PD_Document::getEndTableMutStruxFromTableStrux(const pf_Frag_Strux* table) const
{
	pf_Frag * currentFrag = table->getNext();
	UT_sint32 depth =0;
	while (currentFrag!=m_pPieceTable->getFragments().getLast())
	{
		UT_return_val_if_fail(currentFrag, nullptr);
		if(currentFrag->getType()  == pf_Frag::PFT_Strux)
		{
			auto * pfSec = static_cast<pf_Frag_Strux *>(currentFrag);
			if(pfSec->getStruxType() == PTX_SectionTable)
				depth++;
 ...
 * Returns nullptr on failure to find it.
 */
const pf_Frag_Strux* PD_Document::getEndCellStruxFromCellStrux(const pf_Frag_Strux* cell) const
{
	return getEndCellMutStruxFromCellStrux(cell);
}

pf_Frag_Strux* PD_Document::getEndCellMutStruxFromCellStrux(const pf_Frag_Strux* cell) const
{
	pf_Frag * currentFrag = cell->getNext();
	while (currentFrag && currentFrag!=m_pPieceTable->getFragments().getLast())
	{
		...
		if(currentFrag->getType()  == pf_Frag::PFT_Strux)
		{
			...
			if(pfSec->getStruxType() == PTX_SectionTable)
			{
				pf_Frag_Strux* endTab = getEndTableMutStruxFromTableStrux(pfSec);
				currentFrag = endTab;
			}
 ...
 * Returns nullptr on failure to find it.
 */
const pf_Frag_Strux* PD_Document::getEndTableStruxFromTablePos(PT_DocPosition tablePos) const
{
	const pf_Frag_Strux* tableStrux = nullptr;
	bool bRes =	getStruxOfTypeFromPosition(tablePos, PTX_SectionTable, &tableStrux);
	if(!bRes)
		return nullptr;
	return getEndTableStruxFromTableStrux(tableStrux);
}

/*!
 * The method returns the number of rows and columns in table pointed to by tableSDH
\param pf_Frag_Strux* table Strux of the table in question
...
*/
bool PD_Document::getRowsColsFromTableStrux(const pf_Frag_Strux* table, bool bShowRevisions, UT_uint32 iRevisionLevel,
										  UT_sint32 * numRows, UT_sint32 * numCols) const
{
	UT_sint32 iRight = 0;
    UT_sint32 iBot = 0;
...
// Do the scan
//
	const pf_Frag * currentFrag = table;
	currentFrag = currentFrag->getNext();
	while (currentFrag && currentFrag!=m_pPieceTable->getFragments().getLast())
	{
		UT_return_val_if_fail (currentFrag,0);
		if(currentFrag->getType()  == pf_Frag::PFT_Strux)
		{
			auto pfSec = static_cast<const pf_Frag_Strux *>(currentFrag);
			if(pfSec->getStruxType() == PTX_SectionTable)
			{
//
// skip to the end of this nested table
//
				const pf_Frag_Strux* endStrux = getEndTableStruxFromTableStrux(pfSec);
				pfSec = endStrux;
			}
			else if(pfSec->getStruxType() == PTX_EndTable)
				return true;
			...
			{
				cellStrux = pfSec;
				UT_DebugOnly<bool> bres = getPropertyFromStrux(cellStrux, bShowRevisions, iRevisionLevel, "right-attach", &szRight);
				UT_ASSERT(bres);
				if(szRight && *szRight)
					iRight = atoi(szRight);
				bres = getPropertyFromStrux(cellStrux, bShowRevisions, iRevisionLevel, "bot-attach", &szBot);
				UT_ASSERT(bres);
				if(szBot && *szBot)
					iBot = atoi(szBot);
...
void  PD_Document::miniDump(const pf_Frag_Strux* sdh, UT_sint32 nstruxes) const
{
#ifdef DEBUG
	UT_sint32 i=0;
	...
	for(i=0; pfs && (i< 2*nstruxes); i++)
	{
		...
		const char * szBot = nullptr;
		getPropertyFromStrux(pfs,true, PD_MAX_REVISION,"left-attach",&szLeft);
		getPropertyFromStrux(pfs,true, PD_MAX_REVISION,"right-attach",&szRight);
		getPropertyFromStrux(pfs,true, PD_MAX_REVISION,"top-attach",&szTop);
		getPropertyFromStrux(pfs,true, PD_MAX_REVISION,"bot-attach",&szBot);
		if(szLeft != nullptr)
		{
...
*/
const pf_Frag_Strux* PD_Document::getCellStruxFromRowCol(const pf_Frag_Strux* tableSDH,
													bool bShowRevisions, UT_uint32 iRevisionLevel,
													UT_sint32 row, 
													UT_sint32 col) const
{
	return getCellMutStruxFromRowCol(tableSDH, bShowRevisions, iRevisionLevel, row, col);
}

pf_Frag_Strux* PD_Document::getCellMutStruxFromRowCol(const pf_Frag_Strux* tableSDH,
													bool bShowRevisions, UT_uint32 iRevisionLevel,
													UT_sint32 row, 
													UT_sint32 col) const
{
...
// Do the scan
//
	UT_return_val_if_fail(tableSDH != nullptr, nullptr);

	pf_Frag * currentFrag = tableSDH->getNext();
	while (currentFrag && currentFrag!=m_pPieceTable->getFragments().getLast())
	{
		UT_return_val_if_fail(currentFrag, nullptr);
		if(currentFrag->getType() == pf_Frag::PFT_Strux)
...
// skip to the end of this nested table
//
				pfSec = getEndTableMutStruxFromTableStrux(pfSec);
			}
			else if(pfSec->getStruxType() == PTX_EndTable)
			{
//
...
//
			...
			else if(pfSec->getStruxType() == PTX_SectionCell)
			{
				...
				Right = -1;
				Bot = -1;
				UT_DebugOnly<bool> bres = getPropertyFromStrux(cellSDH,bShowRevisions,iRevisionLevel,"left-attach",&szLeft);
				UT_ASSERT(bres);
				if(szLeft && *szLeft)
					Left = atoi(szLeft);
				bres = getPropertyFromStrux(cellSDH,bShowRevisions,iRevisionLevel,"top-attach",&szTop);
				UT_ASSERT(bres);
				if(szTop && *szTop)
					Top = atoi(szTop);
				bres = getPropertyFromStrux(cellSDH,bShowRevisions,iRevisionLevel,"right-attach",&szRight);
				UT_ASSERT(bres);
				if(szRight && *szRight)
					Right = atoi(szRight);
				bres = getPropertyFromStrux(cellSDH,bShowRevisions,iRevisionLevel,"bot-attach",&szBot);
				UT_ASSERT(bres);
				if(szBot && *szBot)
					Bot = atoi(szBot);
...
size_t PD_Document::getStyleCount(void) const
{
  UT_return_val_if_fail (m_pPieceTable, false);
...
bool PD_Document::tellListener(PL_Listener* pListener) const
{
...
bool PD_Document::tellListenerSubset( PL_Listener* pListener,
                                      ...
                                      PL_ListenerCoupleCloser* closer) const
{
	UT_return_val_if_fail (pListener, false);
	UT_return_val_if_fail (m_pPieceTable, false);
...
const UT_UCS4Char * PD_Document::getPointer(PT_BufIndex bi) const
{
...
bool PD_Document::getBlockBuf(const pf_Frag_Strux* sdh, UT_GrowBuf * pgb) const
{
...
PT_DocPosition PD_Document::getStruxPosition(const pf_Frag_Strux* pfs) const
{
	return m_pPieceTable->getStruxPosition(pfs);
}

bool PD_Document::getSpanAttrProp(const pf_Frag_Strux* sdh, UT_uint32 offset, bool bLeftSide,
...
po_Bookmark * PD_Document::getBookmark(const pf_Frag_Strux* pfsBlock, UT_uint32 offset) const
{
	const pf_Frag * pf = pfsBlock;
	...
	const pf_Frag_Object * pfo = nullptr;
		...
		if (offset < cumOffset)
		{
			switch (pfTemp->getType())
			{
				case pf_Frag::PFT_Object:
					pfo = static_cast<const pf_Frag_Object *>(pfTemp);
...
bool PD_Document::getField(const pf_Frag_Strux* pfsBlock, UT_uint32 offset,
                               fd_Field * & pField) const
{
...
bool PD_Document::getStruxOfTypeFromPosition(PT_DocPosition docPos,
												PTStruxType pts,
												const pf_Frag_Strux* * sdh) const
{
	pf_Frag_Strux *pfs;
	bool result = m_pPieceTable->getStruxOfTypeFromPosition(docPos, pts, &pfs);
	*sdh = pfs;
	return result;
}

bool PD_Document::getMutStruxOfTypeFromPosition(PT_DocPosition docPos,
												PTStruxType pts,
...
///
bool PD_Document::getPrevStruxOfType(const pf_Frag_Strux* pfs, PTStruxType pts,
					const pf_Frag_Strux* * prevsdh) const
{
	pf_Frag_Strux* prevfs = nullptr;
	if (getPrevMutStruxOfType(pfs, pts, &prevfs)) {
		*prevsdh = prevfs;
		return true;
	}
	return false;
...
bool PD_Document::getPrevMutStruxOfType(const pf_Frag_Strux* pfs, PTStruxType pts,
					pf_Frag_Strux* * prevsdh) const
{
	UT_return_val_if_fail (pfs, false);
	for (pf_Frag * pf = pfs->getPrev(); (pf); pf = pf->getPrev())
		if (pf->getType() == pf_Frag::PFT_Strux)
		{
			auto pfsTemp = static_cast<pf_Frag_Strux *>(pf);
...
///
bool PD_Document::getNextStrux(const pf_Frag_Strux* pfs,
							   const pf_Frag_Strux* * nextsdh) const
{
	pf_Frag_Strux* nextpfs = nullptr;
	if (getNextMutStrux(pfs, &nextpfs)) {
		*nextsdh = nextpfs;
		return true;
	}
	return false;
...
bool PD_Document::getNextMutStrux(const pf_Frag_Strux* pfs,
							   pf_Frag_Strux* * nextsdh) const
{
	UT_return_val_if_fail (pfs, false);
	UT_sint32 iEmbedDepth = 0;
	for (pf_Frag * pf = pfs->getNext(); (pf); pf=pf->getNext())
	{
		if (pf->getType() == pf_Frag::PFT_Strux)
		{
...
{
	pf_Frag * pf = nullptr;
	m_pPieceTable->getMutFragFromPosition(docPos, &pf, nullptr);
	return pf;
}
...
///
bool PD_Document::getNextStruxOfType(const pf_Frag_Strux* pfs,PTStruxType pts,
					const pf_Frag_Strux* * nextsdh) const
{
	UT_return_val_if_fail (pfs, false);
	UT_sint32 iNest = 0;
	for (const pf_Frag * pf = pfs->getNext(); (pf); pf = pf->getNext())
		if (pf->getType() == pf_Frag::PFT_Strux)
		{
...
bool PD_Document::getStyleProperty(const char * szStyleName, const char * szPropertyName, const char *& szPropertyValue) const
{
	PD_Style * pS;
	PD_Style ** ppS = &pS;
 ...
 * nullptr
 */
PD_Style * PD_Document::getStyleFromStrux(const pf_Frag_Strux* pfs) const
{
...
*/
const pf_Frag_Strux* PD_Document::getPrevNumberedHeadingStyle(const pf_Frag_Strux* pfs) const
{
	const pf_Frag * pf = pfs;
	bool bFound = false;
	pf = pf->getPrev();
	PD_Style * pStyle = nullptr;
	const pf_Frag_Strux* foundSDH = nullptr;
	...
	while(pf && !bFound)
	{
		if(pf->getType() == pf_Frag::PFT_Strux)
		{
			foundSDH = static_cast<const pf_Frag_Strux*>(pf);
			pStyle = getStyleFromStrux(foundSDH);
			if(pStyle != nullptr)
			{
...
*/
const pf_Frag_Strux* PD_Document::findPreviousStyleStrux(const gchar * szStyle, PT_DocPosition pos) const
{
...
//
	...
	if(bFound)
	{
		sdh = static_cast<const pf_Frag_Strux*>(currentFrag);
	}
	else
	{
		sdh = nullptr;
...
\return the sdh of the strux found.
*/
const pf_Frag_Strux* PD_Document::findForwardStyleStrux(const gchar * szStyle, PT_DocPosition pos) const
{
	const pf_Frag_Strux* sdh = nullptr;
	getStruxOfTypeFromPosition(pos,PTX_Block, &sdh);
	const pf_Frag_Strux * pfs = nullptr;
...
bool PD_Document::enumLists(UT_uint32 k, fl_AutoNumConstPtr & pAutoNum) const
{
...
void PD_Document::listUpdate(const pf_Frag_Strux* pfs) const
{
	//
...
void PD_Document::StopList(const pf_Frag_Strux* pfs)
{
	//
...
bool PD_Document::areListUpdatesAllowed(void) const
{
...
void PD_Document::removeList(const fl_AutoNumPtr & pAutoNum, const pf_Frag_Strux* pfs)
{
...
bool  PD_Document::isDoingPaste(void) const
{
         return m_bDoingPaste;
}

bool PD_Document::convertPercentToInches(const char * szPercent, UT_UTF8String & sInches) const
{
	double width = m_docPageSize.Width(DIM_IN);
	const pf_Frag_Strux* sdhSec = getLastSectionStrux();
	...
	getPropertyFromStrux(sdhSec,true,PD_MAX_REVISION,"page-margin-left",&szLeftMargin);
	getPropertyFromStrux(sdhSec,true,PD_MAX_REVISION,"page-margin-right",&szRightMargin);
...
bool PD_Document::insertStruxBeforeFrag(const pf_Frag * pF, PTStruxType pts,
										const PP_PropertyVector & attributes,
										pf_Frag_Strux ** ppfs_ret)
{
	UT_return_val_if_fail (m_pPieceTable, false);
...
bool PD_Document::insertSpanBeforeFrag(const pf_Frag * pF, const UT_UCS4Char * pbuf, UT_uint32 length)
{
	UT_return_val_if_fail (m_pPieceTable, false);
	if(pF->getType() == pf_Frag::PFT_Strux)
	{
...
bool PD_Document::insertObjectBeforeFrag(const pf_Frag * pF, PTObjectType pto,
										 const PP_PropertyVector & attributes)
{
	UT_return_val_if_fail (m_pPieceTable, false);
	if(pF->getType() == pf_Frag::PFT_Strux)
...
bool PD_Document::insertFmtMarkBeforeFrag(const pf_Frag * pF)
{
	UT_return_val_if_fail (m_pPieceTable, false);
	if(pF->getType() == pf_Frag::PFT_Strux)
	{
 ...
 * to by odh
 */
PT_AttrPropIndex  PD_Document::getAPIFromSOH(pf_Frag_Object* odh) const
{
	pf_Frag_Object * pfo = odh;
	return pfo->getIndexAP();
}	

bool PD_Document::insertFmtMarkBeforeFrag(const pf_Frag * pF, const PP_PropertyVector & attributes)
{
	UT_return_val_if_fail (m_pPieceTable, false);
